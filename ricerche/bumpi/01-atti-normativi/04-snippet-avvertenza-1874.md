...
## Addendum (6 agosto 2026, sera): esito del test diretto dell'API Google Books

Il test diretto da questo ambiente chiarisce che la pista API **non è praticabile senza credenziali proprie**: le richieste a `googleapis.com`/`books.googleapis.com` attraversano il proxy ma vengono attribuite al progetto Google del gateway (`project_number:624717413613`), che ha quota giornaliera **pari a zero** per l'API Books (`quota_limit_value: "0"`, errore 429 RESOURCE_EXHAUSTED). Non si tratta dunque di quota "esaurita oggi": senza una chiave API personale (`&key=...`, gratuita, 1.000 richieste/giorno da Google Cloud Console con Books API abilitata) l'API non risponderà mai da questo ambiente. Con una chiave dell'utente la campagna di snippet-mining sulle digitalizzazioni Google del Bollettino (annate 1874-76, ricerca interna di «Avvertenza», «forza di partecipazione», condizioni di associazione) diventa immediatamente eseguibile.
